...
const SellerProductsPage = () => {
  ...
  try {
    ...
    const productList = await sellerService.getSellerProducts(currentUser.id);
    setProducts(productList);
  } catch {
    setErrorMessage("Unable to load seller products. Please make sure JSON Server is running.");
  } finally {
    setIsLoading(false);
  }
}, [currentUser]); // Now loadProducts only changes if currentUser changes

useEffect(() => {
  void loadProducts();
}, [loadProducts]); // ESLint is happy because loadProducts is now stable
  ...
  const buildPayload = (): SellerProductPayload | null => {
    try {
  ...
  const validatePayload = (payload: SellerProductPayload): boolean => {
  ...
  const handleSubmit = async (
    ...
    if (!currentUser) {
      setErrorMessage("Seller user is not available.");
      return;
    }
    ...
    try {
      ...
      if (editingProductId) {
        const updatedProduct = await sellerService.updateSellerProduct(
          editingProductId,
          currentUser.id,
          payload
        );
        ...
        const createdProduct = await sellerService.createSellerProduct(
          currentUser.id,
          currentUser.name,
          payload
        );
  ...
  const handleDelete = async (productId: string): Promise<void> => {
    ...
    try {
      await sellerService.deleteSellerProduct(productId, currentUser.id);
  ...
  if (isLoading) {
    return <Loader message="Loading seller products..." />;
  }
    ...
    <div>
      <div className="mb-4">
        ...
        <p className="text-muted mb-0">
          Add, update, delete, and manage products owned by your seller account.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="row g-3">
            ...
            <div className="col-12">
              ...
              <textarea
                name="specifications"
                className="form-control seller-code-textarea"
                rows={5}
      ...
      <div className="seller-panel-card">
        <div className="d-flex flex-column flex-md-row justify-content-between gap-3 mb-3">
          ...
          <input
            className="form-control seller-search-input"
